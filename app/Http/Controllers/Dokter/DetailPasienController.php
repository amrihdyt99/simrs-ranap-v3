...
class DetailPasienController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('dokter.pages.detail_pasien.index');
    }
    ...
    public function ajax_index($request)
    {
        ...
        ->editColumn('aksi_data', function ($query) use ($request) {
                return
                ( '<a href="'
                . route('dokter.patient_detail', ['MedicalNo' => $query->MedicalNo])
    ...
    public function show(Request $request, $id)
    {
        ...
        ->first();
        if(!$pasien){
            return redirect()->route('dokter.detail_pasien_list')->with('error_message', 'Pasien Tidak Ditemukan.');
        }
        ...
        return view('dokter.pages.detail_pasien.show', compact('pasien'));
    }
    ...
    public function ajax_index2($request, $id)
    {
        ...
        ->editColumn('aksi_data', function ($query) use ($request) {
                return
                ( '<a href="'
                . route('dokter.patient_detail_registrasi', [$query->reg_no])
    ...
    public function show2(Request $request, $id)
    {
        ...
        ->leftJoin('m_kelas_ruangan_baru', function($join){
            $join->on('m_registrasi.reg_class', '=', 'm_kelas_ruangan_baru.id');
        })
        ...
        return view('dokter.pages.detail_pasien.show_registrasi', compact('registrasi'));
    }
}

...
class DetailPasienPerawatController extends Controller
{
    ...
    public function ajax_index($request)
    {
        ...
        return DataTables()
            ...
            ->editColumn('aksi_data', function ($query) use ($request) {
                return ('<a href="'
                    . route('perawat.patient_detail', ['MedicalNo' => $query->MedicalNo])
    ...
    public function show(Request $request, $id)
    {
        $pasien = DB::connection('mysql2')
            ...
            ->first();
        if (!$pasien) {
            return redirect()->route('perawat.detail_pasien_list')->with('error_message', 'Pasien Tidak Ditemukan.');
        }
    ...
    public function ajax_index2($request, $id)
    {
        ...
        return DataTables()
            ...
            ->editColumn('aksi_data', function ($query) use ($request) {
                return ('<a href="'
                    . route('perawat.patient_detail_registrasi', [$query->reg_no])
    ...
    public function show2(Request $request, $id)
    {
        ...
        $kelas = DB::connection('mysql')
            ...
            ->first();
        if (!$kelas) {
            return redirect()->route('perawat.detail_pasien_list')->with('error_message', 'Kelas Tidak Ditemukan.');
        }
    ...
    public function get_qrcode(Request $request)
    {
        ...
        // response()->json([
        //     'success' => true,
        //     'data' => route('perawat.patient_detail_qrcode', ['date' => $request->date, 'reg_no' => $request->reg_no]),
        // ]);
    }

    public function detail_qrcode(Request $request)
    {
        $cpptData = DB::connection('mysql')
        ...
        $patient = DB::connection('mysql2')
            ->table('m_pasien')
            ->where('MedicalNo', $cpptData->med_rec)->first();

        return view('perawat.pages.detail_pasien.detail_qrcode', compact('cpptData', 'patient'));
    }
}

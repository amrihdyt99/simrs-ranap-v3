...
namespace App\Http\Controllers\NewCaseManager;
...
class DetailPasienCaseManagerController extends Controller
{
    ...
    public function ajax_index($request)
    {
        ...
        ->editColumn('aksi_data', function ($query) use ($request) {
                return
                ( '<a href="'
                . route('casemanager.patient.summary-v2', ['reg_no' => $query->reg_no])
    ...
    public function show(Request $request, $id)
    {
        ...
        ->first();
        if(!$pasien){
            return redirect()->route('casemanager.detail_pasien_list')->with('error_message', 'Pasien Tidak Ditemukan.');
        }
    ...
    public function ajax_index2($request, $id)
    {
        ...
        ->editColumn('aksi_data', function ($query) use ($request) {
                return
                ( '<a href="'
                . route('casemanager.patient_detail_registrasi', [$query->reg_no])
    ...
    public function show2(Request $request, $id)
    {
        ...
        ->first();
        if(!$registrasi){
            return redirect()->route('dietitian.detail_pasien_list')->with('error_message', 'Pasien Tidak Ditemukan.');
        }
        ...
        ->first();
        if(!$kelas){
            return redirect()->route('dietitian.detail_pasien_list')->with('error_message', 'Kelas Tidak Ditemukan.');
        }

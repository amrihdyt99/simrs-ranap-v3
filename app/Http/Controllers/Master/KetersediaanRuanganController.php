...
class KetersediaanRuanganController extends Controller
{
    ...
    public function ajax_index($request)
    {
        ...
        return DataTables()
            ...
            ->editColumn('aksi_data', function ($query) use ($request) {
                return
                ( '<a href="'
                . route('master.ketersediaanruangan.edit', [$query->id])
                . '" class="btn btn-sm"><i class="fas fa-edit text-info"></i></a>' )
                .
                '<form action="'
                . route('master.ketersediaanruangan.destroy', [$query->id])
                . '" method="POST">'
                . csrf_field()
                . method_field('DELETE')
    ...
    {
        DB::connection('mysql2')->table("ketersediaan_ruangan")->where('id', $id)->delete();
        return redirect()->route('master.ketersediaanruangan.index')->with("success", "Data User Berhasil Dihapus.");
    }

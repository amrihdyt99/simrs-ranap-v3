...
class DetailPasienRadiologiController extends Controller
{
    ...
    public function ajax_index($request)
    {
        ...
        ->editColumn('aksi_data', function ($query) use ($request) {
                return
                ( '<a href="'
                . route('radiologi.patient_detail', ['MedicalNo' => $query->MedicalNo])
    ...
    public function show(Request $request, $id)
    {
        ...
        ->first();
        if(!$pasien){
            return redirect()->route('radiologi.detail_pasien_list')->with('error_message', 'Pasien Tidak Ditemukan.');
        }
    ...
    public function ajax_index2($request, $id)
    {
        ...
        ->editColumn('aksi_data', function ($query) use ($request) {
                return
                ( '<a href="'
                . route('radiologi.patient_detail_registrasi', [$query->reg_no])
    ...
    public function show2(Request $request, $id)
    {
        ...
        ->first();
        if(!$kelas){
            return redirect()->route('radiologi.detail_pasien_list')->with('error_message', 'Kelas Tidak Ditemukan.');
        }

        return view('radiologi.pages.detail_pasien.show_registrasi', compact('registrasi','kelas'));
    }
}
